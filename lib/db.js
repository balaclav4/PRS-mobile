...
function owned(row, opts) {
  const [ownerId_, updatedAt, deleted] = stamp(row, opts);
  return { ...row, ownerId: ownerId_, updatedAt, deleted };
}
...
function upsertInto(arr, row, opts) {
  const r = owned(row, opts);
...
// Rows out of SQLite carry their structured fields as JSON strings; every
// put* takes them parsed. Both readers go through these, because the two used
// to map rows separately and drifted: readAllForSync handed raw rows straight
// to sync, so a rifle went up with `torqueJson` and came back into putRifle
// looking for `torque`, which silently wrote NULL. Scope evaluations, zero
// baselines, torque records, chronograph velocities, every load-development
// row and every saved dope card were dropped on each round trip.
const parse = (s, fallback) => { try { return s ? JSON.parse(s) : fallback; } catch { return fallback; } };

const hydrateRifle = (r) => ({
  ...r,
  scopeEval: parse(r.scopeEvalJson, null),
  torque: parse(r.torqueJson, []),
  zeroBaseline: parse(r.zeroBaselineJson, null),
});

const hydrateProject = (p) => ({
  ...p,
  rungs: parse(p.rungsJson, []),
  seatingRows: parse(p.seatingJson, []),
  primerRows: parse(p.primerJson, []),
  workupRows: parse(p.workupJson, []),
  screenRows: parse(p.screenJson, []),
  coarseRows: parse(p.coarseJson, []),
});

const hydrateDopeCard = (c) => ({
  ...c,
  opts: parse(c.optsJson, {}),
  rows: parse(c.rowsJson, []),
});

/** Group target rows by session, in ordinal order. */
function targetsBySession(targetRows) {
  const bySession = new Map();
  for (const t of targetRows) {
    const target = {
      id: t.id,
      scale: parse(t.scaleJson, null),
      shots: parse(t.shotsJson, []),
      aim: parse(t.aimJson, null),
    };
...
const hydrateSession = (s, bySession) => ({
  ...s,
  suppressed: !!s.suppressed,
  velocities: parse(s.velocitiesJson, []),
  targets: bySession.get(s.id) || [],
});

/** The owner/version/tombstone triple, from the sync policy in lib/sync. */
function stamp(rec, opts) { return syncStamp(rec, opts, ownerId); }
...
export async function readAll() {
  ...
  const rifles = rifleRows.map(hydrateRifle);
  ...
  const bySession = targetsBySession(targetRows);
  const sessions = sessionRows.map(s => hydrateSession(s, bySession));
  const projects = projectRows.map(hydrateProject);
  const dopeCards = dopeRows.map(hydrateDopeCard);
...
export async function putRifle(r, opts) {
  if (isWeb) return webMutate(d => upsertInto(d.rifles, r, opts));
  const d = await open();
  const [own, at, del] = stamp(r, opts);
  await d.runAsync(
...
export async function putLoad(l, opts) {
  if (isWeb) return webMutate(d => upsertInto(d.loads, l, opts));
  const d = await open();
  const [own, at, del] = stamp(l, opts);
  await d.runAsync(
...
export async function putSession(s, opts) {
  if (isWeb) return webMutate(d => upsertInto(d.sessions, s, opts));
  const d = await open();
  const [own, at, del] = stamp(s, opts);
  await d.runAsync(
  ...
  //
  // Only when the caller actually brought some. `s.targets` undefined means
  // "this writer does not know about targets", not "this session has none",
  // and the two were treated the same: applyPull handed over a record with no
  // targets field and this deleted every shot on the device, so pulling a
  // session destroyed the copy that had the data. An explicit empty array
  // still clears them, which is what a session edited down to nothing means.
  if (s.targets === undefined) return;
...
export async function putProject(p, opts) {
  if (isWeb) return webMutate(d => {
    d.projects = d.projects || [];
    upsertInto(d.projects, p, opts);
  });
  const d = await open();
  const [own, at, del] = stamp(p, opts);
  await d.runAsync(
...
export async function putDopeCard(c, opts) {
  if (isWeb) return webMutate(d => {
    d.dopeCards = d.dopeCards || [];
    upsertInto(d.dopeCards, c, opts);
  });
  const d = await open();
  const [own, at, del] = stamp(c, opts);
  await d.runAsync(
...
export async function readAllForSync() {
  ...
  const d = await open();
  const [rifleRows, loads, sessionRows, projectRows, dopeRows, targetRows] = await Promise.all([
    ...
    d.getAllAsync('SELECT * FROM targets ORDER BY ordinal'),
  ]);

  // Hydrated, exactly as readAll does it, and for the same reason: whatever
  // comes out of here is what gets written to the server and handed back to
  // put* on the way in. Raw rows round-tripped as JSON strings into fields
  // that put* never reads, so the structured half of every record was thrown
  // away on each sync - and the targets were not fetched at all, which meant a
  // session went up with no shots in it and came back down deleting the shots
  // on the device that had them.
  const bySession = targetsBySession(targetRows);
  return {
    rifles: rifleRows.map(hydrateRifle),
    loads,
    sessions: sessionRows.map(s => hydrateSession(s, bySession)),
    projects: projectRows.map(hydrateProject),
    dopeCards: dopeRows.map(hydrateDopeCard),
  };
}
