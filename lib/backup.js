...
export const BACKUP_FORMAT = 'brass-ballistics-backup';
 ...
 */
export const LEGACY_BACKUP_FORMATS = ['prs-precision-backup', 'on-paper-backup'];
...
export function readBackup(text) {
  ...
  if (parsed.format !== BACKUP_FORMAT && !LEGACY_BACKUP_FORMATS.includes(parsed.format)) {
    return {
      ok: false,
      reason: 'That is not a Brass & Ballistics backup. A session CSV is an export, not a backup — it carries no shots or scale.',
    };
  }
